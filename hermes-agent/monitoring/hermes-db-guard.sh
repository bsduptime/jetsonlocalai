...
#
# Backups land in /var/lib/hermes-db-backups/ (root:dbexpertai mode 750:
# david can list + read them without sudo; only root can write/delete).
# Logs land in /var/log/hermes-db-guard/ (same ownership).
#
# Why this exists:
...
#      for kanban.db.
#   5. Voice-alerts via jetson-voice-say (run as dbexpertai) on
#      integrity_check failure. Does NOT auto-wipe or auto-recover --
...
alert() {
    ...
    # that user's ~/.claude/hooks/ tree.
    if [ -x "$VOICE_HOOK" ]; then
        sudo -u dbexpertai "$VOICE_HOOK" "jetson hermes db guard: $*" \
            >/dev/null 2>&1 &
    fi
}
...
backup_db() {
    ...
    # with the running hermes process).
    if sqlite3 "$src" ".backup '$tmp'" 2>>"$LOG_DIR/guard.log"; then
        local qc
